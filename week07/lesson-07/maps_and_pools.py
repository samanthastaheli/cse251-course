...
def apply_async_with_callback():
    ...
    # TODO using pool.apply, get results after calling square_me. Apply is (was?)
    ...
    # TODO get the results using 'apply_async'.
    # Use apply_async when you need to pass non-iterable values to a function
    pool = mp.Pool(5)
    result = list(pool.apply_async(square_me, args=(i,)) for i in range(101))
                                            # has comma because of pipe, right???
    output = [r.get() for r in result]
    # get is timeout, if can't get moves on, has error message too
    print(f'type={type(result[0])}')
    pool.close()
    pool.join()
    print(f'pool.apply_async:\n{output}\n')

    # TODO get the results using 'apply_async' with callback
    pool = mp.Pool(5)
    for i in range(101):
        pool.apply_async(square_me, args=(i,), callback=callback_function)
    print(f'pool.apply_async (with callback):\n{result_list}\n')

    # TODO Combine map and async. Normal map function is blocking,
